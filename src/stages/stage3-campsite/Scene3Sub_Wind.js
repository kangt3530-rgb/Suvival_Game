...
/** Stage 3 — Scouting 子画面：Wind */
export default class Stage3Scene3SubWindScene extends Phaser.Scene {
  constructor() {
    super({ key: STAGE3_SCENE_KEYS.SCOUTING_SUB_WIND });
  }
  ...
  create() {
    ...
    const bg = this.add.image(cx, cy, STAGE3_ASSETS.BG_SCOUTING_WIND.key).setOrigin(0.5, 0.5);
    setTextureLinearByKey(this, STAGE3_ASSETS.BG_SCOUTING_WIND.key);
    ...
    const rawSub = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_WIND_CHECKED);
    const checkedIds = Array.isArray(rawSub) ? rawSub.slice() : [];
    const windSubIds = Object.keys(SCENE3_WIND_HOTSPOT_CONFIG);
    const resumeSubComplete =
      windSubIds.length > 0 && windSubIds.every((id) => checkedIds.includes(id));
    ...
    const handleBackClick = () => {
      ...
      if (!ml.includes('wind')) ml.push('wind');
      this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_CHECKED, ml);
      this.registry.remove(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_WIND_CHECKED);
    ...
    const handleSubHotspotClick = (id) => {
      const raw = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_WIND_CHECKED);
      const list = Array.isArray(raw) ? raw.slice() : [];
      if (list.includes(id)) return;

      hotspotGroup.markChecked(id);
      if (!list.includes(id)) {
        this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_WIND_CHECKED, [...list, id]);
      }

      const line =
        id === 'sheltered'
          ? "Less wind down here… easier to start a fire, warmer through the night."
          : "Nothing to block the wind… it'll be harder to get a fire going. And colder.";

      enqueueSubVisual(line);
    };

    // 与 Ground 子画面一致：配置为游戏逻辑画布坐标，与 debug 面板一致
    const hotspotsConfig = Object.entries(SCENE3_WIND_HOTSPOT_CONFIG).map(([id, cfg]) => ({
      id,
      x: cfg.x,
      y: cfg.y,
      radius: cfg.radius,
      ...
      onDebugDrag: (newX, newY) => {
        SCENE3_WIND_HOTSPOT_CONFIG[id].x = newX;
        SCENE3_WIND_HOTSPOT_CONFIG[id].y = newY;
      },
    }));
    ...
    const introLine = "Wind's picking up… makes a difference where I sit.";
