...
/** Stage 3 — Scouting 子画面：Ground */
export default class Stage3Scene3SubGroundScene extends Phaser.Scene {
  constructor() {
    super({ key: STAGE3_SCENE_KEYS.SCOUTING_SUB_GROUND });
  }
  ...
  create() {
    ...
    const bg = this.add.image(cx, cy, STAGE3_ASSETS.BG_SCOUTING_GROUND.key).setOrigin(0.5, 0.5);
    setTextureLinearByKey(this, STAGE3_ASSETS.BG_SCOUTING_GROUND.key);
    ...
    const rawSub = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_GROUND_CHECKED);
    const checkedIds = Array.isArray(rawSub) ? rawSub.slice() : [];
    const groundSubIds = Object.keys(SCENE3_GROUND_HOTSPOT_CONFIG);
    const resumeSubComplete =
      groundSubIds.length > 0 && groundSubIds.every((id) => checkedIds.includes(id));
    ...
    const handleBackClick = () => {
      ...
      if (!ml.includes('ground')) ml.push('ground');
      this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_CHECKED, ml);
      this.registry.remove(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_GROUND_CHECKED);
    ...
    const SUB_LINES = {
      low: "Water would collect here if it rains… and cold air settles in low spots too.",
      slope: "The water would run off… but I wouldn't sleep well on this angle.",
      flat: "Flat and dry. Higher up too. This would hold up better if the weather turns.",
    };

    const handleSubHotspotClick = (id) => {
      const raw = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_GROUND_CHECKED);
      const list = Array.isArray(raw) ? raw.slice() : [];
      if (list.includes(id)) return;

      hotspotGroup.markChecked(id);
      if (!list.includes(id)) {
        this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_GROUND_CHECKED, [...list, id]);
      }

      const line = SUB_LINES[id];
      if (line) enqueueSubVisual(line);
    };

    // 与 debug 面板一致：x/y/radius 使用游戏逻辑画布 GAME_WIDTH×GAME_HEIGHT，不做 2560 设计稿缩放
    const hotspotsConfig = Object.entries(SCENE3_GROUND_HOTSPOT_CONFIG).map(([id, cfg]) => ({
      id,
      x: cfg.x,
      y: cfg.y,
      radius: cfg.radius,
      ...
      onDebugDrag: (newX, newY) => {
        SCENE3_GROUND_HOTSPOT_CONFIG[id].x = newX;
        SCENE3_GROUND_HOTSPOT_CONFIG[id].y = newY;
      },
    }));
    ...
    const introLine = 'Three kinds of ground here… let me see how each one would hold up.';
