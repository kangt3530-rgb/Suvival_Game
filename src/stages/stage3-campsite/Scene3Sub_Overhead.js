...
import {
  STAGE3_SCENE_KEYS,
  STAGE3_ASSETS,
  SCENE3_OVERHEAD_HOTSPOT_CONFIG,
...
/** Stage 3 — Scouting 子画面：Overhead */
export default class Stage3Scene3SubOverheadScene extends Phaser.Scene {
  constructor() {
    super({ key: STAGE3_SCENE_KEYS.SCOUTING_SUB_OVERHEAD });
  }
  ...
  create() {
    ...
    const bg = this.add.image(cx, cy, STAGE3_ASSETS.BG_SCOUTING_OVERHEAD.key).setOrigin(0.5, 0.5);
    setTextureLinearByKey(this, STAGE3_ASSETS.BG_SCOUTING_OVERHEAD.key);
    ...
    const rawSub = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_OVERHEAD_CHECKED);
    const checkedIds = Array.isArray(rawSub) ? rawSub.slice() : [];
    const overheadSubIds = Object.keys(SCENE3_OVERHEAD_HOTSPOT_CONFIG);
    const resumeSubComplete =
      overheadSubIds.length > 0 && overheadSubIds.every((id) => checkedIds.includes(id));
    ...
    const handleBackClick = () => {
      ...
      if (!ml.includes('overhead')) ml.push('overhead');
      this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_CHECKED, ml);
      this.registry.remove(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_OVERHEAD_CHECKED);
    ...
    const handleSubHotspotClick = (id) => {
      const raw = this.registry.get(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_OVERHEAD_CHECKED);
      const list = Array.isArray(raw) ? raw.slice() : [];
      if (list.includes(id)) return;

      hotspotGroup.markChecked(id);
      if (!list.includes(id)) {
        this.registry.set(STAGE3_REGISTRY_KEYS.SCOUTING_SUB_OVERHEAD_CHECKED, [...list, id]);
      }

      const line =
        id === 'deadwood'
          ? "Dead wood up there… if the wind gets strong enough tonight, those could come down."
          : "Nothing above me. Safer… but no cover from rain either.";

      enqueueSubVisual(line);
    };

    const hotspotsConfig = Object.entries(SCENE3_OVERHEAD_HOTSPOT_CONFIG).map(([id, cfg]) => ({
      id,
      x: cfg.x,
      y: cfg.y,
      radius: cfg.radius,
      ...
      onDebugDrag: (newX, newY) => {
        SCENE3_OVERHEAD_HOTSPOT_CONFIG[id].x = newX;
        SCENE3_OVERHEAD_HOTSPOT_CONFIG[id].y = newY;
      },
    }));
    ...
    const introLine = "And what's above me… that matters too.";
